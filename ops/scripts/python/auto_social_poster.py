...
class AutoSocialPoster:
    ...
    def __init__(self, config_path: str = "ops/social-automation/config.json"):
    ...
    def _load_social_log(self) -> dict:
        ...
        log_file = Path("ops/scripts/python/.social_posts.json")
        if log_file.exists():
        ...
        if content_path and (content_path.endswith(".md") or content_path.endswith(".html")):
            try:
                ...
                if not full_path.exists():
                    ...
                    full_path = Path(__file__).resolve().parents[3] / content_path
    ...
    def post_new_blogs(self):
        ...
        # Load posts from public/blog/posts.json
        posts_file = Path("public/blog/posts.json")
        if not posts_file.exists():
            logger.warning("public/blog/posts.json not found")
            return
        ...
        if not posts:
            logger.warning("No posts found in public/blog/posts.json")
            return
    ...
    def post_specific_blog(self, slug: str):
        ...
        posts_file = Path("public/blog/posts.json")
        if not posts_file.exists():
            logger.error("public/blog/posts.json not found")
            return
