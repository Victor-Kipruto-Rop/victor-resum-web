...
class ViralDetector:
    ...
    def __init__(self):
        self.performance_file = Path('ops/analytics/performance-scores.json')
        self.viral_history_file = Path('ops/analytics/viral-history.json')
    ...
    def run(self):
        ...
        # Save results
        output_file = Path('ops/analytics/viral-analysis.json')
        with open(output_file, 'w') as f:
            json.dump(results, f, indent=2)
        
        # Generate report
        report = self.generate_viral_report()
        report_file = Path('ops/analytics/viral-report.json')
