...
def test_social_config():
    ...
    try:
        with open("ops/social-automation/config.json") as f:
            config = json.load(f)
...
def test_integration():
    ...
    try:
        ...
        config_path = "ops/social-automation/config.json"
        ...
        if Path("public/posts").exists() and list(Path("public/posts").glob("*.html")):
...
2. Configure .env file:
   cp ops/social-automation/config.json.example .env

3. Run Production Tests:
   python3 ops/tools/test_social_posting.py    # Post to all platforms
   python3 ops/tools/test_email_sending.py     # Send test emails
