...
def main():
    ...
    for email in test_emails:
        ...
        unsubscribe_url = f"https://victor-kipruto-rop.github.io/victor-resum-web/unsubscribe/?token={token}&email={email}"
        print(f"Email: {email}")
        print(f"Token: {token}")
...
4. Unsubscribe URL:
   url = f"https://victor-kipruto-rop.github.io/victor-resum-web/unsubscribe/?token={token}&email={email}"
...
5. Send via Resend API:
   response = requests.post(
    ...
    files = [
        ...
        ("unsubscribe/", "Working unsubscribe success page"),
        ("email-templates-preview/", "Visual preview of all templates"),
