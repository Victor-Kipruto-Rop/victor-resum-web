        ...
        for template_name in TEMPLATES.keys():
            try:
                ...
                if html and len(html) > 500:
                    has_unsubscribe = "unsubscribe/?token=" in html
        ...
        if posts_json.exists():
            try:
                with open(posts_json) as f:
                    ...
                    print_success(f"assets/shared/posts.json loaded: {len(posts_data)} posts")
                    results['posts_json'] = len(posts_data)
            except Exception as e:
                print_error(f"assets/shared/posts.json parsing failed: {e}")
...
def test_unsubscribe_page():
    ...
    try:
        ...
        if Path("unsubscribe/").exists():
            with open("unsubscribe/") as f:
                content = f.read()
            ...
            print_error("unsubscribe/ not found")
...
def test_preview_gallery():
    ...
    try:
        if Path("email-templates-preview/").exists():
            with open("email-templates-preview/") as f:
                content = f.read()
            ...
            print_error("email-templates-preview/ not found")
            results['file_found'] = False
